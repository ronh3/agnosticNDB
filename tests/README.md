...
- `agnosticdb_ui_spec.lua`
  Confirms the help and status views render key sections without throwing, that `qwp` help, validation, and grouped class-mode rendering work with stubbed online data, and that queue-cancel, recent-updates, refresh-online, quick-update, config export/import, transfer export/import, notes, ignore toggling, fetch wrapper, IFF, and elemental-lord flows produce the expected UI output.
